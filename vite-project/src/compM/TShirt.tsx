...
const TShirtItem: React.FC<{title: string, color: string, price: string, image: string}> = ({title, color, price, image}) => {
  ...
  return (<>
      <div className={`TShirt-item-1`}>
        ...
        <p className='TShirt-text-1'>{title}</p>
        <p className='TShirt-text-2'>{color}</p>
        <p className='TShirt-text-3'>{price}</p>
      </div>
  </>)
}
...
function TShirt() {
  ...
  const handleInfoGet = async () => {
    var infoAPI = await handleAPI_GetItem("TShirt");
