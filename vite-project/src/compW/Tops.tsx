...
const TopsItem: React.FC<{title: string, color: string, price: string, image: string}> = ({title, color, price, image}) => {
  ...
  return (<>
      <div className={`Tops-item-1`}>
        ...
        <p className='Tops-text-1'>{title}</p>
        <p className='Tops-text-2'>{color}</p>
        <p className='Tops-text-3'>{price}</p>
      </div>
  </>)
}
...
function Tops() {
  ...
  const handleInfoGet = async () => {
    var infoAPI = await handleAPI_GetItem("Tops");
